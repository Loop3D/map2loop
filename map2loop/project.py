...
class Project(object):
    ...
    def run_all(self, user_defined_stratigraphic_column=None, take_best=False):
        ...
        if issubclass(type(user_defined_stratigraphic_column), list):
            ...
            result = run_map2model(geology_data, fault_data, contact_data)  # if we use a user defined stratigraphic column, we still need to calculate the results of map2model
            fault_fault_relationships = result.get("fault_fault_relationships")
            unit_fault_relationships = result.get("unit_fault_relationships")
            unit_unit_relationships = result.get("unit_unit_relationships")
        else:
            if user_defined_stratigraphic_column is not None:
                logger.warning(
        ...
        self.save_into_projectfile(sampled_contacts_data, fault_fault_relationships, unit_fault_relationships, unit_unit_relationships)

    def save_into_projectfile(self, sampled_contacts_data, fault_fault_relationships, unit_fault_relationships, unit_unit_relationships):
        """
        ...
        if fault_fault_relationships is not None:
