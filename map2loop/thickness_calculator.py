...
_THICKNESS_CALCULATOR_REGISTRY = {}

@beartype.beartype
def register_thickness_calculator(name: str):
    """
    Register a thickness calculator function with a given name.

    Args:
        name (str): the name of the thickness calculator
    """
    def decorator(func):
        _THICKNESS_CALCULATOR_REGISTRY[name] = func
        return func
    return decorator

@beartype.beartype
def get_thickness_calculator(name: str):
    """
    Get a thickness calculator function by name.

    Args:
        name (str): the name of the thickness calculator to retrieve
    """
    if name not in _THICKNESS_CALCULATOR_REGISTRY:
        raise ValueError(f"Thickness calculator {name} not found")
    return _THICKNESS_CALCULATOR_REGISTRY[name]
...
def calculate_thickness(
    ...
    map_data: MapData,
    thickness_calculator_name: str,
    max_line_length: float=None,
    strike_allowance: float=None,
    ...
    **kwargs
    ...
    Args:
        ...
        thickness_calculator (str): the name of the thickness calculator to use
        **kwargs: additional parameters

    Returns:
        Union[dict, pandas.DataFrame]:
        if debug_info is True, returns a dictionary:
            'thicknesses': units dataframe with added thickness column for calculated thickness values
            'lines': the lines
            'location_tracking': location tracking data
        Otherwise, returns:
    ...
    if len(stratigraphic_order) < 3:
        ...
        return units
    calculator = get_thickness_calculator(thickness_calculator_name)

    calculator_args={}
    if max_line_length is not None:
        calculator_args['max_line_length']=max_line_length
    if strike_allowance is not None:
        calculator_args['strike_allowance'] = strike_allowance
    
    calculator_args['debug_info'] = debug_info

    return calculator(
        units, stratigraphic_order, basal_contacts, structure_data, map_data, 
        **calculator_args
    )

@register_thickness_calculator("alpha")
@beartype.beartype
def calculate_thickness_alpha(
    units: pandas.DataFrame,
    stratigraphic_order: list,
    basal_contacts: geopandas.GeoDataFrame,
    structure_data: pandas.DataFrame,
    ...
    **kwargs
    ...
    check_thickness_percentage_calculations(thicknesses,'alpha')
    
    return thicknesses

@register_thickness_calculator("interpolated_structure")
@beartype.beartype
def calculate_thickness_interpolated_structure(
        units: pandas.DataFrame,
        stratigraphic_order: list,
        basal_contacts: geopandas.GeoDataFrame,
        structure_data: pandas.DataFrame,
        ...
        if (
            ...
            if basal_contact is not None and top_contact is not None:
                ...
                for _, row in basal_contact.iterrows():
                    ...
                    if max_line_length is not None and short_line.length > max_line_length:
                        continue
    ...
    location_tracking = combined_location_tracking
    
    # Create GeoDataFrame for lines
    lines = geopandas.GeoDataFrame(geometry=_lines, crs=basal_contacts.crs)
    lines['dip'] = _dips
    
    # Check thickness calculation
    check_thickness_percentage_calculations(thicknesses,'interpolated_structure')
...
@register_thickness_calculator("structural_point")
@beartype.beartype
def calculate_thickness_structural_point(
    units: pandas.DataFrame,
    stratigraphic_order: list,
    basal_contacts: geopandas.GeoDataFrame,
    structure_data: pandas.DataFrame,
    ...
    **kwargs
    ...
    for s in range(0, len(sampled_structures)):
        ...
        B = calculate_endpoints(measurement_pt, strike, max_line_length, bbox_poly)
        ...
        b_s = strike - strike_allowance, strike + strike_allowance
        ...
        if max_line_length is not None and L > max_line_length:
            continue
    ...
    check_thickness_percentage_calculations(output_units,'structural_point')
    
    if debug_info:
        return {
            'thicknesses': output_units,
            'lines': lines
        }
    else:
        return output_units

def check_thickness_percentage_calculations(thicknesses: pandas.DataFrame, calculator_name: str):
    ...
    if total_units > 0 and (units_with_no_thickness / total_units) >= 0.75:
        logger.warning(
            ...
            f"have a calculated thickness of -1. This may indicate that {calculator_name} "
