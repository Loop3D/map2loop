...
def test_colour_units_no_clut_file():
    ...
    assert colours.apply(isinstance, args=(str,)).all(), "function MapData.colour_units without clut file not assigning random hex colours as str"
    assert colours.str.startswith("#").all(), "function MapData.colour_units without clut file not generating the right hex codes with # at the start"
    assert colours.str.len().isin([7, 4]).all(), "function MapData.colour_units without clut file not generating the right hex codes with 7 or 4 characters"
...
def test_colour_units_with_colour_file(tmp_path):
    ...
    assert result["colour"].iloc[0] == expected_colors[0], "function MapData.colour_units with clut file not assigning the right colour from the lookup file"
    assert result["colour"].iloc[1] == expected_colors[1], "function MapData.colour_units with clut file not assigning the right colour from the lookup file"
    assert isinstance(result["colour"].iloc[2], str) , "function MapData.colour_units with clut file not assigning random hex colours as str"
    assert result["colour"].iloc[2].startswith("#"), "function MapData.colour_units with clut file not generating the right hex codes with # at the start"
    assert len(result["colour"].iloc[2]) in {7,4}, "function MapData.colour_units with clut file not generating the right hex codes with 7 or 4 characters"
