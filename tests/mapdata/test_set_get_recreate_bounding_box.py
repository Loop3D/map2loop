...
def test_bounding_box_polygon(md):
    ...
    assert md.bounding_box_polygon.equals(expected_polygon), "MapData.bounding_box_polygon not returning the correct GeoDataFrame"
...
def test_get_bounding_box_as_dict(md):
    ...
    assert result == bounding_box, "MapData.get_bounding_box() not returning the correct bounding box"
...
def test_get_bounding_box_as_polygon(md):
    ...
    assert isinstance(result, geopandas.GeoDataFrame), "MapData.get_bounding_box(polygon=True) not returning a GeoDataFrame"
    assert result.equals(md.bounding_box_polygon), "MapData.get_bounding_box(polygon=True) not returning the correct GeoDataFrame"
...
def test_recreate_bounding_box_str(md):
    ...
    assert md.bounding_box_str == expected_str, "MapData.recreate_bounding_box_str() not working as expected"
...
def test_set_bounding_box_with_missing_keys(md):
    ...
    with pytest.raises(KeyError):
        md.set_bounding_box(bounding_box), "MapData.set_bounding_box accepting wrong argument, but should raise KeyError"
