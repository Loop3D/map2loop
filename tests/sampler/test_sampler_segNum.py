...
geology_original = geopandas.GeoDataFrame(
    geology_original, crs = 'epsg:7854',
)
...
for _, poly in geology_original.iterrows():
    ...
    if poly['geometry'].geom_type == 'Polygon':
        ...
        for _, sample in corresponding_rows.iterrows():
            ...
            assert point.intersects(poly.geometry), f"Point from SegNum 0 is not in the correct polygon segment of ID {poly['ID']}."
    ...
    if poly['geometry'].geom_type == 'MultiPolygon':
        ...
        assert len(multipolygon.geoms) == len(corresponding_rows.segNum.unique()), "Mismatch in the number of geo_polygons and segNum"
        ...
        for i, polygon in enumerate(poly.geometry.geoms):
            ...
            for _, sample in polygon_samples.iterrows():
                point = shapely.Point(sample['X'], sample['Y']).buffer(1) # buffer just to make sure
                assert point.intersects(polygon), f"Point from SegNum {i} is not in the correct polygon segment of ID {poly['ID']}."
