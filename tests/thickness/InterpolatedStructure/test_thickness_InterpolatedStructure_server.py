...
def test_from_aus_state():
    ...
    proj.run_all()
    assert (
        proj.thickness_calculator.sorter_label == "InterpolatedStructure"
    ...
    ), 'Thickness not being calculated in InterpolatedStructureCalculator'
    assert (
        "ThicknessStdDev" in proj.stratigraphic_column.stratigraphicUnits.columns
    ), 'Thickness std not being calculated in InterpolatedStructureCalculator'
